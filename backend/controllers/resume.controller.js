...
export const levelUpdate = async (req, res) => {
  try {
    const { index, isProject, totalAverageScore } = req.body;
const userId = req.id;
    if (!userId || index === undefined || isProject === undefined || totalAverageScore === undefined) {
      return res.status(400).json({ message: "Missing required fields." });
    }
console.log("hereee",userId,index,isProject,totalAverageScore);
    const resume = await Resume.findOne({ userId });
    if (!resume) {
    ...
    if (totalAverageScore > threshold) {
      if (isProject) {
        if (resume.projects[index]) {
          resume.projects[index].isVerified = true;
        } else {
          return res.status(400).json({ message: "Invalid project index." });
        }
      } else {
        if (resume.skills[index]) {
          if (resume.skills[index].level < 3) {
            resume.skills[index].level += 1; // Increase skill level by 1
          }
        } else {
